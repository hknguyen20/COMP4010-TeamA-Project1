...
- **Variables**:  
    ...
    - For both questions, there might be a chance that **external features** beyond the datasets' internal factors may act as confounding factors that bias the results. For example, the popularity of an album might be influenced by marketing efforts, seasonal effects, or global events. If the variables of the dataset are not enough and these external factors indeed carry weight, we will include them as external variables in our analysis. Moreover, while we put in 10+ variables for each question, not all of them may turn out interesting. We only put in potential ones to do explore if there are correlations but in our final write-up we will only show visualization of most prominent variables after a lot of experiments.
...
- **Analysis**:  We will use multiple regression analysis to explore the relationship between dependent variable (album scores) and multiple independent variables (audio features like danceability, etc.).  A more ambitious method would be to use a dimensionality reduction technique such as PCA to select features with the strongest correlation. The choice of plot will heavily depend on the number of resulting variables with strong correlation, so we don't include choice of plot at this proposal stage. However, we may expect to use Heatmap or Scatterplot for Question 1 and Line chart (time series analysis) for Question 2.

- **Interpretation and Conclusion**: The final phase will involve interpreting the results to conclude 1. the musical characteristics that correlate with album success and 2. the shift of Taylor Swift's musical style. This will include a discussion of potential implications for music production and marketing strategies.
